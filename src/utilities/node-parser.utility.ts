...
export class NodeParser {
    ...
    public parseNodeKind(node: ts.Node, templateFilePath: string): void {
        ...
        if (node.kind === ts.SyntaxKind.ClassDeclaration) {
            ...
            if (controllerOptions.isValidController) {
                //
            }
        }
    }
    ...
    private setControllerProperties(members: ts.NodeArray<ts.ClassElement>, templateName: string): ControllerOptions {
        ...
        const template: ts.ClassElement | undefined = this.getTemplatePropertyFromcClassElement(members);
    ...
    private getTemplatePropertyFromcClassElement(members: ts.NodeArray<ts.ClassElement>): ts.ClassElement | undefined {
        return members.find((value: ts.ClassElement) => {
            if (value.name) {
